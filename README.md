...
A production-grade, highly resilient, enterprise **Distributed Job Scheduler** built with Node.js Clean Architecture, TypeScript, Redis, BullMQ, Prisma, and SQLite/PostgreSQL. It features a modern glassmorphic dark-mode React 19 dashboard, AI-driven failure diagnostics powered by the Google Gemini API, sequential/parallel Directed Acyclic Graph (DAG) workflows, automatic worker recovery, and live telemetry tracking.
...
graph TD
    ...
    Worker --> |Failure Diagnostic Prompts| Gemini[Gemini API]
...
* **AI-Driven Failure Diagnostics**: Automatic runtime exception analysis using the Google Gemini API, suggesting actionable markdown fixes for failing jobs directly on the dashboard.
...
* **Diagnostic AI**: Google Gemini API
...
* `GET /api/jobs/:jobId` - Fetch detailed execution history, logs, and AI diagnostics
...
A dark-mode analytics console providing visual tracking of active worker nodes, memory consumption, queue size, and detailed list status logs.

### AI Diagnostics Viewer
When a task fails (e.g. contains `"fail"` in payload), the AI automatically analyzes the stack trace and presents actionable code suggestions.
...
3. **Gemini Diagnostic Key**: If `GEMINI_API_KEY` is omitted, the worker handles failure scenarios seamlessly but skips AI-driven explanation prompts.
